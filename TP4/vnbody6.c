// Code SoA avec structure de vecteur et Optimisation du pow en 1/sqrt() avec implémentation de cette dernière. 
// + optim double boucle for
...
void init(particle_t *p, u64 n)
{
  ...
  p->positions.x = malloc(n * sizeof(f32));
  p->positions.y = malloc(n * sizeof(f32));
  p->positions.z = malloc(n * sizeof(f32));

  p->velocities.x = malloc(n * sizeof(f32));
  p->velocities.y = malloc(n * sizeof(f32));
  p->velocities.z = malloc(n * sizeof(f32));
...
static inline f32 InvSqrt(f32 x)
{
  f32 xhalf = 0.5f*x;
  int i = *(int*)&x; 
  i = 0x5f3759df - (i>>1); 
  ...
  for (u64 i = 0; i < n; i++)
    {
        ...
        int pxi = p->positions.x[i];
        int pyi = p->positions.y[i];
        int pzi = p->positions.z[i];
      ...
      //Newton's law
        d.x[i] = p->positions.x[j] - pxi; //1
        d.y[i] = p->positions.y[j] - pyi; //2 
        d.z[i] = p->positions.z[j] - pzi; //3
      
      
	    const f32 d_2 = (d.x[i] * d.x[i]) + (d.y[i] * d.y[i]) + (d.z[i] * d.z[i]) + softening; //9
      
	    f32 d_3_over_2 = InvSqrt(d_2); //11
        d_3_over_2 = d_3_over_2 * d_3_over_2 * d_3_over_2; //13

	  //Net force
	    f.x[i] += d.x[i] * d_3_over_2; //15
	    f.y[i] += d.y[i] * d_3_over_2; //17
	    f.z[i] += d.z[i] * d_3_over_2; //19
	}

      //
      p->velocities.x[i] += dt * f.x[i];//21
      p->velocities.y[i] += dt * f.y[i];//23
      p->velocities.z[i] += dt * f.z[i];//25
    }
